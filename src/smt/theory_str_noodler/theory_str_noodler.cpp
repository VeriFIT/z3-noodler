...
namespace smt::noodler {
    ...
    theory_var theory_str_noodler::mk_var(enode* const n) {
        if (!m_util_s.is_seq(n->get_expr()) && !m_util_s.is_re(n->get_expr())) {
    ...
    bool theory_str_noodler::internalize_atom(app* const atom, const bool gate_ctx) {
        (void)gate_ctx;
        STRACE(str,
               tout << "internalize_atom: gate_ctx is " << gate_ctx << ", " << mk_pp(atom, get_manager()) << '\n';);
    ...
    bool theory_str_noodler::internalize_term(app* const term) {
        ...
        if (ctx.e_internalized(term)) {
            ...
            e = ctx.mk_enode(term, false, m_manager.is_bool(term), true);
        }
        mk_var(e);
        if (!ctx.relevancy()) {
            relevant_eh(term);
    ...
    void theory_str_noodler::init_search_eh() {
        ...
        for (unsigned i = 0; i < nFormulas; ++i) {
            STRACE(str_init_formula, tout << "Initial asserted formula " << i << ": "
                                          << expr_ref(ctx.get_asserted_formula(i), m_manager) << std::endl;);
            expr* ex = ctx.get_asserted_formula(i);
            this->input_has_quantifiers |= util::has_quantifiers(m_manager, ex);
            if (!add_len_num_axioms(ex)) {
                obj_hashtable<app> lens;
                util::get_len_exprs(ctx.get_asserted_formula(i), m_util_s, m_manager, lens);
    ...
    void theory_str_noodler::string_theory_propagation(expr* ex, bool init, bool neg, bool var_lengths) {
        ...
        if (init && m_manager.is_eq(ex) && neg) {
            ctx.mark_as_relevant(m_manager.mk_not(ex));
        }
        ...
        if (init &&
            (m_util_s.str.is_prefix(ex) || m_util_s.str.is_suffix(ex) || m_util_s.str.is_contains(ex) ||
             m_util_s.str.is_is_digit(ex) || m_util_s.str.is_lt(ex)
        ...
        if (init &&
            (m_util_s.str.is_index(ex) || m_util_s.str.is_at(ex) || m_util_s.str.is_extract(ex) ||
             m_util_s.str.is_replace(ex) || m_util_s.str.is_replace_all(ex) || m_util_s.str.is_replace_re_all(ex))) {
            ctx.mark_as_relevant(ex);
        }
    ...
    void theory_str_noodler::propagate_concat_axiom(enode* cat) {
        ...
        STRACE(str_concat, tout << "[Concat Axiom] " << mk_pp(len_xy, m) << " = " << mk_pp(len_x, m) << " + "
                                << mk_pp(len_y, m) << std::endl;);
    ...
    void theory_str_noodler::propagate_basic_string_axioms(enode* str, bool var_lengths) {
...

        {
            ...
            sort* str_sort = m_util_s.str.mk_string_sort();
            if (a_sort != str_sort) {
                STRACE(str, tout << "WARNING: not setting up string axioms on non-string term "
                                 << mk_pp(str->get_expr(), m) << std::endl;);
                return;
            }
        }
    ...
    void theory_str_noodler::add_length_axiom(expr* n) {
        app_ref ln(m_util_a.mk_ge(n, m_util_a.mk_int(0)), m_manager);
    ...
    void theory_str_noodler::relevant_eh(app* const n) {
        STRACE(str, tout << "relevant: " << mk_pp(n, get_manager()) << " with family id " << n->get_family_id()
                         << ", sort " << n->get_sort()->get_name() << " and decl kind " << n->get_decl_kind()
                         << std::endl;);
        ...
        if (m_util_s.str.is_length(n)) {  // str.len
            ...
            var_name.insert({util::get_variable_basic_term(n), expr_ref(n, m_manager)});
        } else if (m_util_s.str.is_concat(n) ||  // str.++
                   m_util_s.re.is_to_re(n) ||    // str.to_re
                   m_util_s.str.is_in_re(n) ||   // str.in_re
                   m_util_s.is_re(
                       n) ||  // one of re. command (re.none, re.all, re.comp, ...), or possibly RegLan variable
    ...
    */
    void theory_str_noodler::enforce_length(expr* e) {
        ...
        do {
            ...
            if (!has_length(o) && !m_manager.is_ite(o)) {
                expr_ref len = expr_ref(m_util_s.str.mk_length(o), m_manager);
    ...
    void theory_str_noodler::new_eq_eh(theory_var x, theory_var y) {
        ...
        expr_ref l {get_enode(x)->get_expr(), m_manager};
        expr_ref r {get_enode(y)->get_expr(), m_manager};

        STRACE(str, tout << "new_eq: " << l << " = " << r << std::endl;);

        app* equation = m_manager.mk_eq(l, r);
        ...
        if (m_util_s.is_re(l) && m_util_s.is_re(r)) {  // language equation
            ...
            if (m_manager.is_false(equation_atom)) {
                // if we have two distinct literals, we immediately stop by not allowing this equation
                add_axiom({mk_literal(m_manager.mk_not(equation))});
            } else if (!m_manager.is_true(equation_atom)) {
                ...
                if (ctx.is_relevant(equation) || ctx.is_relevant(m_manager.mk_eq(r, l))) {
    ...
    void theory_str_noodler::new_diseq_eh(theory_var x, theory_var y) {
        ...
        const expr_ref l {get_enode(x)->get_expr(), m_manager};
        const expr_ref r {get_enode(y)->get_expr(), m_manager};

        app* equation = m_manager.mk_eq(l, r);
        app* disequation = m_manager.mk_not(equation);

        // This is to handle the case containing ite inside disequations
        // TODO explain better
        if (!ctx.e_internalized(equation)) {
            STRACE(str, tout << "relevanting: " << mk_pp(disequation, m_manager) << std::endl;);
        ...
        if (m_util_s.is_re(l) && m_util_s.is_re(r)) {  // language disequation
            ...
            // string literals)
            app* equation_atom = ctx.mk_eq_atom(l, r);
            if (m_manager.is_true(equation_atom)) {
            ...
            } else if (!m_manager.is_false(equation_atom)) {
                // if equation is not trivially false, we add it for later check
                m_word_diseq_todo.push_back({l, r});
            }
        }

        STRACE(str, tout << ctx.find_assignment(equation) << " " << ctx.find_assignment(disequation) << std::endl
                         << "new_diseq: " << l << " != " << r << " @" << m_scope_level << " "
                         << ctx.get_bool_var(equation) << " " << ctx.is_relevant(disequation) << ":"
                         << ctx.is_relevant(equation) << std::endl;);
    }
    ...
    void theory_str_noodler::pop_scope_eh(const unsigned num_scopes) {
        ...
        STRACE(str, tout << "pop_scope: " << num_scopes << " (back to level " << m_scope_level << ")\n";);
    ...
    void theory_str_noodler::add_axiom(expr* const e) {
        ...
        if (!axiomatized_terms.contains(e)) {
            ...
            if (e == nullptr || get_manager().is_true(e)) {
                return;
            }
            context& ctx = get_context();
    ...
    void theory_str_noodler::add_axiom(std::vector<literal> ls) {
        STRACE(
            str_axiom, tout << "add_axiom literals:" << std::endl; literal_vector lv; for (const auto& l : ls) {
    ...
    void theory_str_noodler::handle_char_at(expr* e) {
        ...
        STRACE(str, tout << "handle char_at: " << mk_pp(e, m_manager) << '\n';);

        expr *s = nullptr, *i = nullptr;
        VERIFY(m_util_s.str.is_at(e, s, i));

        expr_ref v = get_fresh_var_for_string_function("at", e);
        expr_ref v_in_allchar(m_util_s.re.mk_in_re(v, m_util_s.re.mk_full_char(nullptr)), m_manager);
        expr_ref zero(m_util_a.mk_int(0), m_manager);
        expr_ref one(m_util_a.mk_int(1), m_manager);
        ...
        expr_ref emp(m_util_s.str.mk_empty(e->get_sort()), m_manager);                                // empty string
        ...
        if (zstring str; m_util_s.str.is_string(s, str) && str.length() == 1) {
            // i = 0 -> v = "A"
            add_axiom({~mk_literal(m_manager.mk_eq(i, m_util_a.mk_int(0))), mk_eq(v, s, false)});
            // i != 0 -> v = eps
            add_axiom({mk_literal(m_manager.mk_eq(i, m_util_a.mk_int(0))), mk_eq(v, emp, false)});
            return;
        }

        const int MAX_SMALL_INT =
            20;  // threshold for small integers used in the next two cases (experimentally chosen)
        ...
        //   |s| <= i -> v = eps
        if (rational num;
            m_util_a.is_numeral(i, num) && num.is_nonneg() &&
            num < MAX_SMALL_INT) {  // we want small integer, because otherwise we will create a lot of equations
        ...
        //   i < 0 -> v = eps
        if (rational num;
            expr_cases::is_num_plus_len(i, s, m_manager, m_util_s, m_util_a, num) && num.is_neg() &&
            num > -MAX_SMALL_INT) {  // we want small integer, because otherwise we will create a lot of equations
            int val = num.get_int32();
            ...
            for (int j = val; j < 0; j++) {
                ...
                y = m_util_s.str.mk_concat(
                    y, m_util_s.str.mk_at(s, m_util_a.mk_add(m_util_a.mk_int(j), m_util_s.str.mk_length(s))));
            }
            string_theory_propagation(y);
        ...
        expr_ref y = mk_str_var_fresh("at_right");
        expr_ref xvy(m_util_s.str.mk_concat(x, v, y), m_manager);
        string_theory_propagation(xvy);

        expr_ref len_x(m_util_s.str.mk_length(x), m_manager);
    ...
    void theory_str_noodler::handle_substr(expr* e) {
        ...
        STRACE(str, tout << "handle substr: " << mk_pp(e, m_manager) << '\n';);
        ...
        expr_ref zero(m_util_a.mk_int(0), m_manager);
        expr_ref one(m_util_a.mk_int(1), m_manager);
        expr_ref eps(m_util_s.str.mk_string(""), m_manager);
        expr_ref re_allchar(m_util_s.re.mk_full_char(nullptr), m_manager);

        expr_ref lv(m_util_s.str.mk_length(v), m_manager);
        expr_ref ls(m_util_s.str.mk_length(s), m_manager);
        expr_ref ls_minus_i_minus_l(mk_sub(mk_sub(ls, i), l), m_manager);
        ...
        const unsigned MAX_LOOPING =
            50;  // maximal looping allowed in regexes (if too large, the automata get too large)
        ...
        if (zstring indexof_find_string;
            m_util_a.is_zero(i) &&
            expr_cases::is_one_add_indexof_string(l, s, m_manager, m_util_s, m_util_a, indexof_find_string) &&
            !indexof_find_string.empty()) {
            literal indexof_did_not_find =
                mk_eq(l, zero, false);  // if (1 + (str.indexof s t n))==0, then t was not found in s from position n
            // we get the indexof expr by substracting 1 from l
            expr_ref indexof(m_util_a.mk_add(l, m_util_a.mk_int(-1)), m_manager);
            m_rewrite(indexof);
            // (str.substr s 0 (str.indexof s t n))
            expr_ref new_substr(m_util_s.str.mk_substr(s, i, indexof), m_manager);
            // t[0]
            expr_ref first_char_of_t(m_util_s.str.mk_string(indexof_find_string.extract(0, 1)), m_manager);
        ...
        //   s = eps -> v = eps
        if (rational num; m_util_a.is_zero(i) &&
                          expr_cases::is_num_plus_len(l, s, m_manager, m_util_s, m_util_a, num) && num.is_minus_one()) {
            expr_ref y = mk_str_var_fresh("post_substr");
            expr_ref y_in_allchar(m_util_s.re.mk_in_re(y, m_util_s.re.mk_full_char(nullptr)), m_manager);
            expr_ref ly(m_util_s.str.mk_length(y), m_manager);
            expr_ref vy(m_util_s.str.mk_concat(v, y), m_manager);
            ...
            mark_expression_as_length(s);
            this->var_eqs.add(expr_ref(l, m_manager), v);
            return;
        }
        ...
        if (m_util_a.is_zero(i)) {
            ...
            if (expr* n, * t; m_util_a.is_add(i, n, t)) {  // is i of the form t+n?
                if (rational n_value; m_util_a.is_numeral(n, n_value) && n_value.is_pos() && n_value <= MAX_LOOPING) {
                    ...
                    // x2 in re.allchar^n
                    expr_ref x2_in_sigma_times_num(
                        m_util_s.re.mk_in_re(
                            x2, m_util_s.re.mk_loop_proper(re_allchar, n_value_unsigned, n_value_unsigned)),
                        m_manager);
                    ...
                    this->var_eqs.add(
                        expr_ref(t, m_manager), x1,
                        false);  // equivalence might not be precise (we need to check whether the equivalence holds in final_check).
            ...
            this->var_eqs.add(
                expr_ref(i, m_manager),
                x);  // |x| = i, is not true for i <0, but because x is fresh and is used only in s=xvy, we only care about the situation where s=xvy is true, and in that case i>=0
            ...
            if (rational i_val; m_util_a.is_numeral(i, i_val) && i_val.is_pos() && i_val <= MAX_LOOPING) {
                ...
                expr_ref x_in_sigma_time_i(
                    m_util_s.re.mk_in_re(x, m_util_s.re.mk_loop_proper(re_allchar, i_val_unsigned, i_val_unsigned)),
                    m_manager);
                // x in re.allchar^i
        ...
        expr_ref y(m_manager);
        // i+l >= |s|
        expr_ref post_bound(m_util_a.mk_ge(m_util_a.mk_add(i, l), m_util_s.str.mk_length(s)), m_manager);
        m_rewrite(post_bound);  // simplify
        if (m_manager.is_true(post_bound)) {
            ...
            y = expr_ref(m_util_s.str.mk_string(""), m_manager);
        } else {
            ...
            if (rational l_val; m_util_a.is_numeral(l, l_val) && l_val.is_pos() && l_val <= MAX_LOOPING) {
                ...
                expr_ref substr_in(
                    m_util_s.re.mk_in_re(v, m_util_s.re.mk_loop_proper(re_allchar, l_val_unsigned, l_val_unsigned)),
                    m_manager);
    ...
    void theory_str_noodler::handle_replace(expr* r) {
        ...
        STRACE(str, tout << "handle replace: " << mk_pp(r, m_manager) << '\n';);
        ...
        // s = eps -> |v| = |a| + |t|
        add_axiom({~s_emp,
                   mk_literal(m_manager.mk_eq(m_util_s.str.mk_length(v),
                                              m_util_a.mk_add(m_util_s.str.mk_length(a), m_util_s.str.mk_length(t))))});
        ...
        if (expr_cases::is_replace_indexof(a, s, m_manager, m_util_s, m_util_a, indexof)) {
            expr_ref minus_one(m_util_a.mk_int(-1), m_manager);
            expr_ref eps(m_util_s.str.mk_string(""), m_manager);
            literal ind_eq_m1 = mk_eq(indexof, minus_one, false);
            expr_ref len_a_m1(m_util_a.mk_sub(m_util_s.str.mk_length(a), m_util_a.mk_int(1)), m_manager);
            expr_ref substr(m_util_s.str.mk_substr(a, m_util_a.mk_int(0), len_a_m1), m_manager);
        ...
        if (m_util_s.str.is_string(a, str_a) && str_a.length() == 1) {
            ...
            add_axiom({~s_emp, mk_literal(m_manager.mk_eq(v, mk_concat(t, a)))});
            add_axiom({mk_literal(m_manager.mk_not(m_manager.mk_eq(s, eps))),
                       mk_literal(m_manager.mk_eq(v, mk_concat(t, a)))});
            // s = a -> v = t
            // NOTE: if we use ~mk_eq(s, a), this diseqation does not become relevant
            add_axiom({mk_literal(m_manager.mk_not(m_manager.mk_eq(s, a))), mk_eq(v, t, false)});
            ...
            add_axiom({~cnt, mk_literal(m_manager.mk_not(m_manager.mk_eq(s, a))), mk_eq(v, t, false)});
            ...
            // s = emp -> v = t.a
            add_axiom({mk_literal(m_manager.mk_not(m_manager.mk_eq(s, eps))), mk_eq(v, t, false)});
            // s = emp -> v = t.a
        ...
        if (m_util_s.str.is_string(s, str_a) && m_util_s.str.is_string(t, str_b) && str_a.length() >= str_b.length()) {
            add_axiom(
                {~cnt, mk_literal(m_manager.mk_eq(m_util_s.str.mk_length(v),
                                                  m_util_a.mk_sub(m_util_s.str.mk_length(a),
                                                                  m_util_a.mk_int(str_a.length() - str_b.length()))))});
        }
    ...
    void theory_str_noodler::handle_index_of(expr* i) {
        ...
        STRACE(str, tout << "handle indexof: " << mk_pp(i, m_manager) << '\n';);
        ...
        if (!offset || m_util_a.is_zero(offset)) {
            ...
            expr_ref xsy(m_util_s.str.mk_concat(x, s, y), m_manager);
            string_theory_propagation(xsy);

            expr_ref lenx(m_util_s.str.mk_length(x), m_manager);
            // s = eps -> indexof = 0
            ...
            if (expr_cases::is_indexof_at(s, t, m_manager, m_util_s)) {
                add_axiom({mk_literal(m_util_a.mk_ge(i, zero))});
            }

            // update length variables
            this->len_vars.insert(x);
            this->var_eqs.add(expr_ref(i, m_manager), x);

            expr_ref rest(m_util_a.mk_sub(m_util_a.mk_sub(m_util_s.str.mk_length(t), i), m_util_s.str.mk_length(s)),
                          m_manager);
            m_rewrite(rest);
            this->var_eqs.add(rest, y);

        } else {
            expr_ref len_t(m_util_s.str.mk_length(t), m_manager);
            ...
            expr_ref xy(m_util_s.str.mk_concat(x, y), m_manager);
            string_theory_propagation(xy);

            expr_ref indexof0(m_util_s.str.mk_index(y, s, zero), m_manager);
            expr_ref offset_p_indexof0(m_util_a.mk_add(offset, indexof0), m_manager);
    ...
    void theory_str_noodler::tightest_prefix(expr* s, expr* x, std::vector<literal> neg_assumptions) {
        ...
        if (m_util_s.str.is_string(s, str)) {
            if (str.length() == 0) {
                ...
                expr_ref s1(m_util_s.str.mk_string(str.extract(0, str.length() - 1)), m_manager);
            ...
            neg_assumptions.push_back(mk_literal(m_manager.mk_eq(s, s1s2)));
            add_axiom(neg_assumptions);

            // not(s = eps) -> neg_assumptions || s2 in re.allchar (is a single character)
            expr_ref re(m_util_s.re.mk_in_re(s2, m_util_s.re.mk_full_char(nullptr)), m_manager);
    ...
    void theory_str_noodler::handle_replace_re(expr* e) {
        ...
        STRACE(str, tout << "handle replace_re: " << mk_pp(e, m_manager) << '\n';);
    ...
    void theory_str_noodler::handle_replace_all(expr* e) {
        ...
        STRACE(str, tout << "handle replace_all: " << mk_pp(e, m_manager) << '\n';);
    ...
    void theory_str_noodler::handle_replace_re_all(expr* e) {
        ...
        STRACE(str, tout << "handle replace_re_all: " << mk_pp(e, m_manager) << '\n';);
    ...
    expr_ref theory_str_noodler::mk_concat(expr* e1, expr* e2) {
        return expr_ref(m_util_s.str.mk_concat(e1, e2), m_manager);
    }

    literal theory_str_noodler::mk_eq_empty(expr* _e) {
        context& ctx = get_context();
        expr_ref e(_e, m_manager);
        SASSERT(m_util_s.is_seq(e));
        zstring s;
        if (m_util_s.str.is_empty(e)) {
            return true_literal;
        }
        expr_ref_vector concats(m_manager);
    ...
    void theory_str_noodler::handle_prefix(expr* e) {
        ...
        STRACE(str, tout << "handle prefix: " << mk_pp(e, m_manager) << '\n';);
        ...
        if (m_util_s.str.is_string(x, str) && str.length() == 1 &&
            m_util_s.str.is_extract(y, sub_str, sub_ind, sub_len) && m_util_a.is_numeral(sub_ind) &&
            m_util_a.is_numeral(sub_len, val) && val.get_int32() >= 1) {
        ...
        expr_ref xs(m_util_s.str.mk_concat(x, fresh), m_manager);
    ...
    void theory_str_noodler::handle_not_prefix(expr* e) {
        if (axiomatized_persist_terms.contains(m_manager.mk_not(e))) {
            return;
        }
        axiomatized_persist_terms.insert(m_manager.mk_not(e));

        STRACE(str, tout << "handle not(prefx): " << mk_pp(e, m_manager) << '\n';);
        ...
        if (m_util_s.str.is_string(x, str)) {
            expr_ref re(
                m_util_s.re.mk_in_re(y, m_util_s.re.mk_concat(m_util_s.re.mk_to_re(m_util_s.str.mk_string(str)),
        ...
        if (m_util_s.str.is_string(y, str)) {
            ...
            for (size_t i = 0; i <= str.length(); i++) {
                ...
                add_axiom({lit_e, mk_literal(m_manager.mk_not(m_manager.mk_eq(x, m_util_s.str.mk_string(substr))))});
            }
            return;
        }
        ...
        if (m_util_s.str.is_at(y)) {
            literal lit_e = mk_literal(e);
            expr_ref eps(m_util_s.str.mk_string(""), m_manager);
            add_axiom({lit_e, mk_literal(m_manager.mk_not(m_manager.mk_eq(x, y)))});
            add_axiom({lit_e, mk_literal(m_manager.mk_not(m_manager.mk_eq(x, eps)))});
            return;
        }
        ...
        expr_ref pmx(m_util_s.str.mk_concat(p, mx), m_manager);
        string_theory_propagation(pmx);
        expr_ref pmxqx(m_util_s.str.mk_concat(pmx, qx), m_manager);
        string_theory_propagation(pmxqx);
        expr_ref pmy(m_util_s.str.mk_concat(p, my), m_manager);
        string_theory_propagation(pmy);
        expr_ref pmyqy(m_util_s.str.mk_concat(pmy, qy), m_manager);
        string_theory_propagation(pmyqy);

        // |x| <= |y|
        expr_ref len_x_gt_len_y(m_manager);
        zstring s;
        if (m_util_s.str.is_string(x, s)) {
            len_x_gt_len_y =
                expr_ref {m_util_a.mk_ge(m_util_s.str.mk_length(y), m_util_a.mk_int(s.length())), m_manager};
        } else {
            // we cannot put |x| <= |y| because lra solver gives up, we need to give it |y| - |x| >= 0 (there has to be a numeral on one side)
            len_x_gt_len_y =
                expr_ref {m_util_a.mk_ge(m_util_a.mk_sub(m_util_s.str.mk_length(y), m_util_s.str.mk_length(x)),
                                         m_util_a.mk_int(0)),
                          m_manager};
        }
        ...
        literal eq_mx_my = mk_literal(m_manager.mk_not(ctx.mk_eq_atom(mx, my)));

        expr_ref rex(m_util_s.re.mk_in_re(mx, m_util_s.re.mk_full_char(nullptr)), m_manager);
        expr_ref rey(m_util_s.re.mk_in_re(my, m_util_s.re.mk_full_char(nullptr)), m_manager);

        literal lit_e = mk_literal(e);
        literal len_y_gt_len_x = mk_literal(m_manager.mk_not(len_x_gt_len_y));
        // not(e) && |x| <= |y| -> x = p.mx.qx
        ...
        this->var_eqs.add(expr_ref(m_util_a.mk_int(1), m_manager), expr_ref(my, m_manager));
        this->var_eqs.add(expr_ref(m_util_a.mk_int(1), m_manager), expr_ref(mx, m_manager));
    }
    ...
    void theory_str_noodler::handle_suffix(expr* e) {
        ...
        STRACE(str, tout << "handle suffix: " << mk_pp(e, m_manager) << '\n';);

        expr *x = nullptr, *y = nullptr;
        VERIFY(m_util_s.str.is_suffix(e, x, y));

        expr_ref fresh = mk_str_var_fresh("suffix");
        expr_ref px(m_util_s.str.mk_concat(fresh, x), m_manager);
    ...
    void theory_str_noodler::handle_not_suffix(expr* e) {
        ...
        STRACE(str, tout << "handle not(suffix): " << mk_pp(e, m_manager) << '\n';);
        ...
        if (m_util_s.str.is_string(x, str)) {
            expr_ref re(
                m_util_s.re.mk_in_re(y, m_util_s.re.mk_concat(m_util_s.re.mk_star(m_util_s.re.mk_full_char(nullptr)),
        ...
        expr_ref len_x_gt_len_y {
            m_util_a.mk_gt(m_util_a.mk_sub(m_util_s.str.mk_length(x), m_util_s.str.mk_length(y)), m_util_a.mk_int(0)),
            m_manager};
        literal len_y_gt_len_x = mk_literal(len_x_gt_len_y);
        expr_ref pxmx(m_util_s.str.mk_concat(px, mx), m_manager);
        string_theory_propagation(pxmx);
        expr_ref pxmxq(m_util_s.str.mk_concat(pxmx, q), m_manager);
        string_theory_propagation(pxmxq);
        expr_ref pymy(m_util_s.str.mk_concat(py, my), m_manager);
        string_theory_propagation(pymy);
        expr_ref pymyq(m_util_s.str.mk_concat(pymy, q), m_manager);
        ...
        this->var_eqs.add(expr_ref(m_util_a.mk_int(1), m_manager), my);
        this->var_eqs.add(expr_ref(m_util_a.mk_int(1), m_manager), mx);
    }
    ...
    void theory_str_noodler::handle_contains(expr* e) {
        ...
        STRACE(str, tout << "handle contains: " << mk_pp(e, m_manager) << std::endl;);
        ...
        expr* ind = nullptr;
        zstring str;
        if (expr_cases::is_contains_index(e, ind, m_manager, m_util_s, m_util_a)) {
            expr_ref ind_eq(m_manager.mk_eq(ind, m_util_a.mk_int(-1)), m_manager);
            expr_ref ind_leq(m_util_a.mk_le(ind, m_util_a.mk_int(-1)), m_manager);
            literal not_e = mk_literal(mk_not({e, m_manager}));
            ...
            expr_ref re(m_util_s.re.mk_to_re(m_util_s.str.mk_string("")), m_manager);
            for (size_t i = 0; i < str.length(); i++) {
                re = m_util_s.re.mk_union(re, m_util_s.re.mk_to_re(m_util_s.str.mk_string(str[i])));
            }
            expr_ref in_re(m_util_s.re.mk_in_re(y, re), m_manager);
            literal not_e = mk_literal(mk_not({e, m_manager}));
        ...
        expr_ref pys(m_util_s.str.mk_concat(m_util_s.str.mk_concat(p, y), s), m_manager);
    ...
    void theory_str_noodler::handle_not_contains(expr* e) {
        STRACE(str, tout << "handle not(contains) " << mk_pp(e, m_manager) << std::endl;);

        expr* cont = this->m_manager.mk_not(e);
        expr *x = nullptr, *y = nullptr;
        VERIFY(m_util_s.str.is_contains(e, x, y));

        zstring s;
        if (m_util_s.str.is_string(y, s)) {
            expr_ref re(m_util_s.re.mk_in_re(
                            x, m_util_s.re.mk_concat(
                                   m_util_s.re.mk_star(m_util_s.re.mk_full_char(nullptr)),
            ...
            m_util_s.str.is_string(x, s) &&
            s.length() <=
                10) {  // the number 10 is arbitrary, can be tuned (needs to be changed also in assign_not_contains)
            ...
            for (unsigned i = 0; i < s.length(); i++) {
                for (unsigned j = i + 1; j <= s.length(); j++) {
                    expr_ref substr(m_util_s.re.mk_to_re(m_util_s.str.mk_string(s.extract(i, j - i))), m_manager);
                    re = m_util_s.re.mk_union(re, substr);
                }
            }
            re = expr_ref(m_util_s.re.mk_in_re(y, re), m_manager);
            add_axiom({mk_literal(e), ~mk_literal(re)});
        }
    }
    ...
    void theory_str_noodler::assign_not_contains(expr* e) {
        STRACE(str, tout << "assign not(contains) " << mk_pp(e, m_manager) << std::endl;);
        ...
        if (!m_util_s.str.is_string(y) && !(m_util_s.str.is_string(x, s) && s.length() <= 10)) {
            m_not_contains_todo.push_back({{x, m_manager}, {y, m_manager}});
        }
    }
    ...
    void theory_str_noodler::handle_lex_lt(expr* e) {
        STRACE(str, tout << "handle lessthan: " << mk_pp(e, m_manager) << std::endl;);

        expr *x = nullptr, *y = nullptr;
        VERIFY(m_util_s.str.is_lt(e, x, y));
        expr_ref eps(m_util_s.str.mk_string(""), m_manager);
        expr_ref x_eps(m_manager.mk_eq(x, eps), m_manager);
        expr_ref y_eps(m_manager.mk_eq(y, eps), m_manager);
        ...
        expr_ref px(m_util_s.str.mk_concat(m_util_s.str.mk_concat(lex_pre, lex_in_left), lex_post_left), m_manager);
        expr_ref py(m_util_s.str.mk_concat(m_util_s.str.mk_concat(lex_pre, lex_in_right), lex_post_right), m_manager);
        string_theory_propagation(px);
        string_theory_propagation(py);

        expr_ref x_px(m_manager.mk_eq(x, px), m_manager);
        expr_ref y_py(m_manager.mk_eq(y, py), m_manager);
        literal lit_e = mk_literal(e);
        literal lit_x_px = mk_literal(x_px);
        literal lit_y_py = mk_literal(y_py);

        expr_ref re_in_left(m_util_s.re.mk_in_re(lex_in_left, m_util_s.re.mk_full_char(nullptr)), m_manager);
        expr_ref re_in_right(m_util_s.re.mk_in_re(lex_in_right, m_util_s.re.mk_full_char(nullptr)), m_manager);
        expr_ref to_code_left(m_util_s.str.mk_to_code(lex_in_left), m_manager);
        expr_ref to_code_right(m_util_s.str.mk_to_code(lex_in_right), m_manager);
        ...
        expr_ref to_code_lt(m_manager.mk_eq(m_util_a.mk_add(to_code_left, vark), to_code_right), m_manager);
        // k >= 1
...
>>>>>>> 17cf8d230785a44cc389c66e0908afffcc6fbb93
    ...
    void theory_str_noodler::handle_in_re(expr* const e, const bool is_true) {
        STRACE(str, tout << "handle in_re " << mk_pp(e, m_manager) << " " << is_true << std::endl;);
        ...
        app_ref re_constr(to_app(s), m_manager);
        expr_ref re_atom(e, m_manager);
        ...
        if (re_constr->get_num_args() != 0) {
            ...
            if (this->predicate_replace.contains(re_constr)) {
                var = expr_ref(this->predicate_replace[re_constr], m_manager);
            } else {
            ...
            expr_ref eq_fv(mk_eq_atom(var.get(), s), m_manager);
            expr_ref n_re(this->m_util_s.re.mk_in_re(var, re), m_manager);
            expr_ref re_orig(e, m_manager);

            // propagate_basic_string_axioms(ctx.get_enode(eq_fv));

            if (!is_true) {
                n_re = m_manager.mk_not(n_re);
                re_orig = m_manager.mk_not(re_orig);
            }
        ...
        expr_ref r {re, m_manager};
        this->m_membership_todo.push_back(std::make_tuple(expr_ref(re_constr, m_manager), r, is_true));
    }
    ...
    void theory_str_noodler::handle_is_digit(expr* e) {
        ...
        STRACE(str, tout << "handle is_digit: " << mk_pp(e, m_manager) << '\n';);

        expr* s = nullptr;
        VERIFY(m_util_s.str.is_is_digit(e, s));
        // s \in [0-9]
        expr_ref s_in_digit(
            m_util_s.re.mk_in_re(s, m_util_s.re.mk_range(m_util_s.str.mk_string("0"), m_util_s.str.mk_string("9"))),
            m_manager);
        // is_digit(s) -> s \in [0-9]
    ...
    void theory_str_noodler::add_conversion_num_axioms() {
        ...
        for (unsigned i = 0; i < nFormulas; ++i) {
            ...
            if (expr_cases::is_to_int_num_eq(ex, m_manager, m_util_s, m_util_a, to_int_arg, val) && val.is_nonneg()) {
                expr_ref re(
                    m_util_s.re.mk_concat(m_util_s.re.mk_star(m_util_s.re.mk_to_re(m_util_s.str.mk_string("0"))),
                                          m_util_s.re.mk_to_re(m_util_s.str.mk_string(val.to_string()))),
                    m_manager);
                expr_ref in_re(m_util_s.re.mk_in_re(to_int_arg, re), m_manager);
    ...
    bool theory_str_noodler::add_len_num_axioms(expr* ex) {
        ...
        rational val;
        bool val_is_larger;
        expr_ref_vector len_arg(m_manager);
        if (expr_cases::is_len_num_eq(ex, m_manager, m_util_s, m_util_a, len_arg, val) && val < MAX_NUM) {
            if (val < 0) {
                ...
                expr_ref re(m_util_s.re.mk_full_char(nullptr), m_manager);
                for (rational i {1}; i < val; i++) {
                    re = m_util_s.re.mk_concat(re, m_util_s.re.mk_full_char(nullptr));
                }
                expr_ref in_re(m_util_s.re.mk_in_re(m_util_s.str.mk_concat(len_arg, nullptr), re), m_manager);
                add_axiom({~mk_literal(ex), mk_literal(in_re)});
                return true;
            }
        } else if (expr_cases::is_len_num_leq_or_geq(ex, m_manager, m_util_s, m_util_a, len_arg, val, val_is_larger) &&
                   val < MAX_NUM) {
            if (val < 0) {
                if (val_is_larger) {
                ...
                expr_ref re(val_is_larger
                                ? m_util_s.re.mk_loop(m_util_s.re.mk_full_char(nullptr), m_util_a.mk_int(0),
                                                      m_util_a.mk_int(val))
    ...
    void theory_str_noodler::handle_conversion(expr* conversion) {
        STRACE(str, tout << "handle conversion " << mk_pp(conversion, m_manager) << std::endl;);
        expr* arg = nullptr;
        ...
        if (tranforming_from) {
            ...
            var_name.insert({var_for_arg, expr_ref(arg, m_manager)});
        } else {
            // the argument has string type, we have to find the variable for it
            expr_ref z3_var_for_arg(m_manager);
            if (m_util_s.str.is_string(arg)) {
                ...
                add_axiom({mk_literal(m_manager.mk_eq(arg, z3_var_for_arg))});
            } else {
        ...
        if (tranforming_from) {
            ...
            add_axiom({mk_literal(m_manager.mk_eq(z3_var_for_conversion, conversion))});
            this->predicate_replace.insert(conversion, z3_var_for_conversion);
            len_vars.insert(
                z3_var_for_conversion);  // we need exact solution for the result, to compute the arithmetic formula
            ...
            if (type == ConversionType::FROM_INT) {
                ...
                add_axiom(
                    {mk_literal(m_util_s.re.mk_in_re(z3_var_for_conversion, m_util_s.re.mk_union(all_nums, epsilon)))});

                // |from_int(x)| = 0 <-> x <= -1
                add_axiom({mk_literal(m_manager.mk_eq(m_util_s.str.mk_length(conversion), m_util_a.mk_int(0))),
                           ~mk_literal(m_util_a.mk_le(arg, m_util_a.mk_int(-1)))});
                add_axiom({~mk_literal(m_manager.mk_eq(m_util_s.str.mk_length(conversion), m_util_a.mk_int(0))),
                           mk_literal(m_util_a.mk_le(arg, m_util_a.mk_int(-1)))});
                ...
                add_axiom({~mk_literal(m_util_a.mk_le(
                               arg, m_util_a.mk_int(
                                        rational(10).expt(m_params.m_underapprox_length) -
                                        1))),  // I rather use <= instead of <, LIA solver can have problems with that
                           mk_literal(m_util_s.re.mk_in_re(
                               z3_var_for_conversion,
                               m_util_s.re.mk_loop(m_util_s.re.mk_full_char(nullptr), m_util_a.mk_int(0),
                                                   m_util_a.mk_int(m_params.m_underapprox_length))))});
            }

            if (type == ConversionType::FROM_REAL) {
                if (width_for_rtos > 0) {
                    app* digits_restricted_by_width = m_util_s.re.mk_loop(
                        m_util_s.re.mk_range(m_util_s.str.mk_string("0"), m_util_s.str.mk_string("9")),
                        m_util_a.mk_int(width_for_rtos), m_util_a.mk_int(width_for_rtos));
                    app* all_nums_with_decimal_part_restricted_by_width = m_util_s.re.mk_concat(
                        all_nums, m_util_s.re.mk_concat(m_util_s.re.mk_to_re(m_util_s.str.mk_string(".")),
                                                        digits_restricted_by_width));
                    ...
                    add_axiom({mk_literal(m_util_s.re.mk_in_re(
                        z3_var_for_conversion,
                        m_util_s.re.mk_union(all_nums_with_decimal_part_restricted_by_width, epsilon)))});
                } else {
                    // the decimal part must be empty, so the result of str.from_real must be a whole number (or empty string)
                    add_axiom({mk_literal(
                        m_util_s.re.mk_in_re(z3_var_for_conversion, m_util_s.re.mk_union(all_nums, epsilon)))});
                }

                // |from_real(x)| = 0 <-> x < 0
                add_axiom({mk_literal(m_manager.mk_eq(m_util_s.str.mk_length(conversion), m_util_a.mk_int(0))),
                           mk_literal(m_util_a.mk_le(m_util_a.mk_real(0), arg))});
                add_axiom({~mk_literal(m_manager.mk_eq(m_util_s.str.mk_length(conversion), m_util_a.mk_int(0))),
                           ~mk_literal(m_util_a.mk_le(m_util_a.mk_real(0), arg))});
                ...
                // the case "argument >= 10^m_underapprox_length" will not happen .
                add_axiom({~mk_literal(m_util_a.mk_le(
                               arg, m_util_a.mk_real(
                                        rational(10).expt(m_params.m_underapprox_length) -
                                        1))),  // I rather use <= instead of <, LIA solver can have problems with that
                           mk_literal(m_util_s.re.mk_in_re(
                               z3_var_for_conversion,
                               m_util_s.re.mk_loop(
                                   m_util_s.re.mk_full_char(nullptr), m_util_a.mk_int(0),
                                   m_util_a.mk_int(rational(m_params.m_underapprox_length + 1) + width_for_rtos))))});
            }
        } else {
            ...
            }  // probably not needed, we put it in var_name anyway, so its type does not matter
            var_name.insert({var_for_conversion, expr_ref(conversion, m_manager)});
            ...
            if (type == ConversionType::TO_REAL) {
                ...
                add_axiom({mk_literal(m_util_a.mk_le(m_util_a.mk_real(0), conversion)),
                           mk_literal(m_manager.mk_eq(m_util_a.mk_real(-1), conversion))});
    ...
    void theory_str_noodler::set_conflict(const literal_vector& lv) {
        context& ctx = get_context();
        const auto& js =
            ext_theory_conflict_justification {get_id(), ctx, lv.size(), lv.data(), 0, nullptr, 0, nullptr};
